...
public class Testing2 {
    ...
    public Testing2(double bias1, double bias2, double bias3, double[] alfa_1, double[] alfa_2, double[] alfa_3) {
        data_training1 = new double[92][18];
        data_training2 = new double[68][18];
        data_training3 = new double[47][18];
        data_testing = new double[30][18];
        baca_testing("E:\\DataTesting.xls");
        baca_training("E:\\datatraining.xls", "E:\\datatraininglevel 2.xls", "E:\\datatraininglevel 3.xls");
        this.bias1 = bias1;
        this.bias2 = bias2;
        this.bias3 = bias3;
        this.alfa_1 = alfa_1;
    ...
    public double[] Kxix(int idxTraining, int idxTesting) {
        ...
        } else if (a.pilihKernel() == 2) {
            System.out.println("Kernel Polynomial Degree 2");
            for (int i = 0; i < data_training.length; i++) {
                kernel[i] = ((Math.pow((data_testing[idxTesting][0] * data_training[i][0]) + (data_testing[idxTesting][1] * data_training[i][1])
                        + (data_testing[idxTesting][2] * data_training[i][2]) + (data_testing[idxTesting][3] * data_training[i][3]) + (data_testing[idxTesting][4] * data_training[i][4]) + (data_testing[idxTesting][5] * data_training[i][5])
                        + (data_testing[idxTesting][6] * data_training[i][6]) + (data_testing[idxTesting][7] * data_training[i][7]) + (data_testing[idxTesting][8] * data_training[i][8]) + (data_testing[idxTesting][9] * data_training[i][9])
                        + (data_testing[idxTesting][10] * data_training[i][10]) + (data_testing[idxTesting][11] * data_training[i][11]) + (data_testing[idxTesting][12] * data_training[i][12])
                        + (data_testing[idxTesting][13] * data_training[i][13]) + (data_testing[idxTesting][14] * data_training[i][14]) + (data_testing[idxTesting][15] * data_training[i][15]) + C, 2)));
            }
            System.out.println("Kxix - " + idxTesting);
            for (int i = 0; i < kernel.length; i++) {//
                System.out.print(kernel[i] + "|");
        ...
        } else if (a.pilihKernel() == 3) {
            System.out.println("Kernel Polynomial of Degree");
            for (int i = 0; i < data_training.length; i++) {
                for (int j = 0; j < data_training.length; j++) {
                    kernel[i] = ((data_testing[idxTesting][0] * data_training[i][0]) + (data_testing[idxTesting][1] * data_training[i][1])
                            + (data_testing[idxTesting][2] * data_training[i][2]) + (data_testing[idxTesting][3] * data_training[i][3]) + (data_testing[idxTesting][4] * data_training[i][4]) + (data_testing[idxTesting][5] * data_training[i][5])
                            + (data_testing[idxTesting][6] * data_training[i][6]) + (data_testing[idxTesting][7] * data_training[i][7]) + (data_testing[idxTesting][8] * data_training[i][8]) + (data_testing[idxTesting][9] * data_training[i][9])
                            + (data_testing[idxTesting][10] * data_training[i][10]) + (data_testing[idxTesting][11] * data_training[i][11]) + (data_testing[idxTesting][12] * data_training[i][12])
                            + (data_testing[idxTesting][13] * data_training[i][13]) + (data_testing[idxTesting][14] * data_training[i][14]) + (data_testing[idxTesting][15] * data_training[i][15]));
                }
            }
            System.out.println("Kxix - " + idxTesting);
            for (int i = 0; i < kernel.length; i++) {//
...
//        }
    ...
    public double level1(int idxTesting) {
        ...
        System.out.println("Hasil LEVEL 1 = " + hasil);
        return hasil;
    }
    ...
    public double level2(int idxTesting) {
        ...
        double hasil = 0;
        hasil = Math.signum(fx);
        System.out.println("Hasil LEVEL 2 = " + hasil);
        return hasil;
    }
    ...
    public double level3(int idxTesting) {
        ...
        double hasil = 0;
        hasil = Math.signum(fx);
        System.out.println("Hasil LEVEL 3 = " + hasil);
        return hasil;
    }
//
...
//    }
    ...
    public double Test() {
        double hasilUji[][] = new double[data_testing.length][2];//kolom pertama buat urutan, kolom kedua untuk hasil tiap levelnya
        for (int i = 0; i < data_testing.length; i++) {
